...
class Dependency(gclient_utils.WorkItem, DependencySettings):
  ...
  def LateOverride(self, url):
    ...
    if isinstance(url, basestring):
      if url.startswith("git@") and url.endswith(".git") :
        parsed_urls = url.split("@")
        parsed_url = []
        parsed_url.append( parsed_urls[0] + "@" + parsed_urls[1] )
        parsed_urls.pop(0)
        parsed_urls.pop(0)
        parsed_url.extend( parsed_urls )
      else:
